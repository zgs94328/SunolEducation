...
public abstract class GSYVideoView extends GSYTextureRenderView implements GSYMediaPlayerListener {
    ...
    public void onError(int what, int extra) {
        ...
        if (mNetChanged) {
            Debuger.printfError(this.getClass().getSimpleName(), "DMLog*******onError() mNetChanged = " + mNetChanged);
            mNetChanged = false;
            netWorkErrorLogic();
            if (mVideoAllCallBack != null) {
                mVideoAllCallBack.onPlayError(mOriginUrl, VideoAllCallBack.ErrorTypeNet, mTitle, this);
            }
            return;
        }

        Debuger.printfError(this.getClass().getSimpleName(), "DMLog*******onError() what = " + what);
        if (what != 38 && what != -38) {
            setStateAndUi(CURRENT_STATE_ERROR);
            deleteCacheFileWhenError();
            if (mVideoAllCallBack != null) {
                mVideoAllCallBack.onPlayError(mOriginUrl, VideoAllCallBack.ErrorTypeUnknow, mTitle, this);
            }
        }
    }
