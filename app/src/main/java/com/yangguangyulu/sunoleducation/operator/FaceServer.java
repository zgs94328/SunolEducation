...
import com.yangguangyulu.sunoleducation.model.face.FaceRegisterInfo;
import com.yangguangyulu.sunoleducation.util.ImageUtil;
...
import static com.yangguangyulu.sunoleducation.base.Constants.faceConstants.SAVE_FEATURE_DIR;
import static com.yangguangyulu.sunoleducation.base.Constants.faceConstants.SAVE_FEATURE_NAME;
import static com.yangguangyulu.sunoleducation.base.Constants.faceConstants.SAVE_IMG_DIR;
...
public class FaceServer {
    ...
    private static FaceRegisterInfo faceRegisterInfo;
    ...
    private void initFaceList(Context context) {
        synchronized (this) {
            ...
            File featureFile = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR + File.separator + SAVE_FEATURE_NAME);
            ...
            try {
                ...
                fis.read(feature);
                fis.close();
                faceRegisterInfo = new FaceRegisterInfo(feature, featureFile.getName());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public boolean register(Context context, byte[] nv21, int width, int height) {
        synchronized (this) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                String userName = SAVE_FEATURE_NAME;
                try {
                    //3.保存注册结果（注册图、特征数据）
                    if (code == ErrorInfo.MOK) {
                        ...
                        //内存中的数据同步
                        if (faceRegisterInfo == null) {
                            faceRegisterInfo = new FaceRegisterInfo(faceFeature.getFeatureData(), userName);
                        }
                        return true;
                    }
                } catch (IOException e) {
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        if (faceEngine == null || isProcessing || faceFeature == null || faceRegisterInfo == null) {
            return null;
        }
        ...
        isProcessing = true;
        tempFaceFeature.setFeatureData(faceRegisterInfo.getFeatureData());
